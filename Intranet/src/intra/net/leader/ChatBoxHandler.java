...
if (chk.equals("intial"))
{
String strt =	inputStream.readUTF();
	if (strt.equals("start"))
	{
		while (!strt.equals("end"))
		{
			
			strt = inputStream.readUTF();
			
			if (!strt.equals("end"))
			{
				newusr = strt;
				usrList.add(newusr);
				IndividualControler.inst.add(newusr);
			}
